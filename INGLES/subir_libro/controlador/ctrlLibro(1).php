<?php
    ...
    if(isset($_POST['subir'])){
        if ($_FILES['archivo']['type']=='application/pdf') {
            if($_FILES['portada']['type']=='image/png' || $_FILES['portada']['type']=='image/jpeg'){
                ...
                if($archivo!="" ) {
                    if (copy ($ruta, $destino) && copy ($ruta2, $destino2)){
                        ...
                        ?> <script>alert('We will check the book, to publish it!')
                            window.location='../vista/vistaLibro.php';</script> 
                        <?php  
                            
                    }else{
                        ?> <script>alert('The book could not be uploaded correctly...')</script> 
                        <?php 
                    }
                }
            }else if ($_FILES['portada']['type']!='image/png' ||  $_FILES['portada']['type']!='image/jpeg' ){
                ?>  
                 <script>alert('PNG or JPG file required');
            ...
            ?>  
              <script>alert('PDF document required for your book');
                window.history.back(1);
              </script> 
            <?php  
        }
    ...
    if($id != "" && $accion=="eliminar"){
        ...
        foreach ($archivos as $archivo){
            ...
            $daoEli->eliminar($id);
            ?>
            <script>alert('The next book has been deleted: <?php echo $tituloLB?>');
