<?php 
    ...
    if($accion == "Modificar"){
        ...
        if(strlen($nombreUsu)>0 && strlen($nombre)>0 && strlen($email)>0){
            ...
            $msj = $mensaje; 
            ?> 
            <script>alert("User modified correctly, log in again...")
            window.location='../../index.php';</script>
            <?php
        }else{
            ?>
            <script>alert("There are empty fields...");
            window.history.go(-1);</script> 
        <?php
        }
    }
    ...
    if($accion=="Confirmar"){
        ...
        if($_FILES['foto']['tmp_name']==''){
            ...
            ?> 
            <script>alert("Photo modified correctly...")
